	...
	<div class='flex sm:flex-row flex-col items-center justify-between mt-4'>
		{#if data.post.authorId === data.session?.user?.id}
			<div class='join'>
				...
				<a class='join-item btn btn-primary' href='/post/{data.id}/download?version={data.post.version}'
					 target='_blank'>
		...
		{:else }
			<a class='btn btn-primary' href='/post/{data.id}/download?version={data.post.version}' target='_blank'>
				<Icon icon='ic:baseline-cloud-download' class='inline' />
				Download
			</a>
		{/if}
		<div>
			...
			<p> create:  {new Date(data.post.createdAt).toLocaleDateString()} | last update:  {new Date(data.post.updatedAt).toLocaleDateString()}</p>
		</div>
	</div>
	<a
		...
		<div class='card-body flex flex-row p-2'>
			<figure class='avatar rounded-full h-16'><img src={data.post.authorIcon} alt='icon' /></figure>
			<div>
