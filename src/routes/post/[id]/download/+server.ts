...
export const GET = (async ({ params, url ,locals}) => {
	...
	const post = await locals.db.select({
		...
		file: contents.file
	}).from(posts).where(eq(posts.id, id)).leftJoin(contents, and(
		eq(posts.id, contents.post),
		eq(posts.version, contents.version)
	)).get();
	if (!post || !post.file) throw error(404, 'Post not found');
	await locals.db.update(posts).set({ download: post.download + 1 }).where(eq(posts.id, id));
